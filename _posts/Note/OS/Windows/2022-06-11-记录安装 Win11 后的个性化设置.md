...
## Windows11 LTSC 安装 OneDrive

1. 去微软下载 OneDrive 并安装

2. LTSC 默认没有 OneDrive，并且安装后会有 Bug（缺失 `{A52BBA46-E9E1-435f-B3D9-28DAA648C0F6}`），需要手动添加注册表相关内容，`LTSC_ONEDRIVE_FOLDER.reg` 是从专业工作站版直接导出的注册表相关内容，直接导入即可
