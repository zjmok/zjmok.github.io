...
| 选项 | 描述 | 适用场景 |
|------|------|----------|
| `no` | 不自动重启容器（默认值） | 测试环境、一次性任务 |
| `always` | 总是重启容器，无论退出状态如何 | 关键服务（如数据库、GitLab） |
| `on-failure` | 仅在非0退出状态时重启 | 预期可能失败但需要自动恢复的服务 |
| `unless-stopped` | 总是重启，除非用户显式停止容器 | 生产环境常用设置 |

- docker 命令 `--restart always`
